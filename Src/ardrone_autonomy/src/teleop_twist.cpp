...
geometry_msgs::Twist posecmd_vel,finalcmd_vel,yawcmd_vel;
geometry_msgs::TwistStamped teleopcmd_vel;

bool needs_takeoff = false;
bool needs_land = false;
bool needs_reset = false;
geometry_msgs::Twist cmd_vel;
geometry_msgs::Twist cmd_yaw_vel;
...
void posecmdVelCallback(const geometry_msgs::TwistConstPtr &msg)
{
        vp_os_mutex_unlock(&twist_lock);
	const float maxHorizontalSpeed = 1; // use 0.1f for testing and 1 for the real thing
	posecmd_vel.linear.x  = max(min(-msg->linear.x, maxHorizontalSpeed), -maxHorizontalSpeed);
	posecmd_vel.linear.y  = max(min(-msg->linear.y, maxHorizontalSpeed), -maxHorizontalSpeed);
	posecmd_vel.linear.z  = max(min(msg->linear.z, 1), -1);
	posecmd_vel.angular.x = 0;
	posecmd_vel.angular.y = 0;
	posecmd_vel.angular.z = max(min(-msg->angular.z, 1), -1);
        vp_os_mutex_unlock(&twist_lock);
}

void yawcmdVelCallback(const geometry_msgs::TwistConstPtr &msg)
{
    //ROS_INFO("yawcallback");
	yawcmd_vel.angular.z = msg->angular.z; //max(min(msg->angular.z, 1), -1);
}
